import AsyncStorage from "@react-native-async-storage/async-storage";
  ...
  export async function loadServerUrl(): Promise<string> {
    try {
      ...
      cachedBaseUrl = stored?.trim() ? ensureProtocol(stored) : DEFAULT_URL;
    } catch {
  ...
  async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
    ...
    const res = await fetch(`${cachedBaseUrl}${path}`, { ...options, headers });
    if (!res.ok) {
  ...
  export const checkMaintenance = () =>
    fetch(`${cachedBaseUrl}/api/admin/check-maintenance`)
      .then((r) => r.json())
  ...
  export const acceptFriend = (id: number) =>
    request<{ message: string }>(`/api/friends/requests/${id}/accept`, { method: "POST" });

  export const rejectFriend = (id: number) =>
    request<{ message: string }>(`/api/friends/requests/${id}/reject`, { method: "POST" });
  ...
  export const adminLogin = (username: string, kata_sandi: string) =>
    fetch(`${cachedBaseUrl}/api/admin/login`, {
      ...
      if (!r.ok) return r.json().then((d) => Promise.reject(new Error((d as { error?: string }).error || "Login gagal")));
